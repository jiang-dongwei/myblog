...
#ifndef FIGHTPAD12SLIM_AMBIENT_GP22_LOOPBACK_DIAGNOSTIC
#define FIGHTPAD12SLIM_AMBIENT_GP22_LOOPBACK_DIAGNOSTIC 0
#endif

#ifndef FIGHTPAD12SLIM_BUTTON_LEDS_FORCE_BOARD_DEFAULTS
#define FIGHTPAD12SLIM_BUTTON_LEDS_FORCE_BOARD_DEFAULTS 0
#endif

#ifndef FIGHTPAD12SLIM_AMBIENT_OWNS_GP22
#define FIGHTPAD12SLIM_AMBIENT_OWNS_GP22 0
#endif
...
#if FIGHTPAD12SLIM_AMBIENT_GP22_LOOPBACK_DIAGNOSTIC
    return false;
#endif
#if FIGHTPAD12SLIM_AMBIENT_OWNS_GP22
    return false;
#endif
#if FIGHTPAD12SLIM_BUTTON_LEDS_FORCE_BOARD_DEFAULTS
    return isValidPin(BOARD_LEDS_PIN);
#endif
...
void NeoPicoLEDAddon::setup() {
    ...
    ledOptions.ledFormat = static_cast<LEDFormat_Proto>(LED_FORMAT);
    ...
    ledOptions.pledType = PLED_TYPE_NONE;
...
void NeoPicoLEDAddon::process() {
    ...
    const TurboOptions& turboOptions = Storage::getInstance().getAddonOptions().turboOptions;
    Gamepad * gamepad = Storage::getInstance().GetProcessedGamepad();
    Gamepad * liveGamepad = Storage::getInstance().GetGamepad();
    GamepadHotkey action = animationHotkeys(liveGamepad);
    if (ledOptions.pledType == PLED_TYPE_RGB) {
    ...
    uint32_t buttonState = liveGamepad->state.dpad << 16 | liveGamepad->state.buttons;
    ...
    // Fightpad12Slim requirement: keep all WS2812 LEDs solid white.
    uint32_t white = ColorWhite.value(neopico.GetFormat(), as.GetBrightnessX());
    int maxLeds = (ledCount > FRAME_MAX) ? FRAME_MAX : ledCount;
...
#if defined(FIGHTPAD12SLIM)
    // The stock shortcut treats S1 as BACK. Fightpad's physical BACK/Share key
    // is GP19/A2, so accept either prefix without changing the board GPIO map.
    const bool pressedBackStart = gamepad->pressedS2() && (gamepad->pressedS1() || gamepad->pressedA2());
    const uint32_t hotkeyMask = GAMEPAD_MASK_S1 | GAMEPAD_MASK_S2 | GAMEPAD_MASK_A2;
#else
    const bool pressedBackStart = gamepad->pressedS1() && gamepad->pressedS2();
    const uint32_t hotkeyMask = GAMEPAD_MASK_S1 | GAMEPAD_MASK_S2;
#endif
    if (pressedBackStart)
    {
